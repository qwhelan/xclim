...
daily_temperature_range_variability = \
    TasminTasmax(identifier='dtrvar',
                 units='K',
                 standard_name='air_temperature',
                 long_name='Mean Diurnal Temperature Range Variability',
                 description='{freq} mean diurnal temperature range variability, defined as the average day-to-day '
                             'magnitude of change in daily temperature range.',
...
extreme_temperature_range = \
    TasminTasmax(identifier='etr',
                 units='K',
                 standard_name='air_temperature',
                 long_name='Intra-period Extreme Temperature Range',
                 description='{freq} range between the maximum of daily maximum temperature '
                             'and the minimum of daily minimum temperature.',
                 compute=_ind.extreme_temperature_range,
                 )

cold_spell_duration_index = \
    Tasmin(identifier='csdi_{window}',
           units='days',
           standard_name='cold_spell_duration_index',
           long_name='Cold Spell Duration Index',
           description='{freq} number of days with at least {window} consecutive days where the daily minimum '
                       'temperature is below the 10th percentile. The 10th percentile should be computed for '
                       'a 5-day moving window centred on each calendar day in the 1961-1990 period.',
           cell_methods='',
...
cold_spell_days = Tas(identifier='csi_{thresh}',
                      ...
                      description='{freq} number of days that are part of a cold spell, defined as {window} '
                                  'or more consecutive days with mean daily temperature below {thresh}°C.',
                      cell_methods='',
                      compute=_ind.cold_spell_days,
                      )

daily_freezethaw_cycles = \
    TasminTasmax(identifier='dlyfrzthw',
                 units='days',
                 standard_name='daily_freezethaw_cycles',
                 long_name='daily freezethaw cycles',
                 description='{freq} number of days with a diurnal freeze-thaw cycle. A cycle occurs when the minimum '
                             'daily temperature plunges below 0℃ and the maximum daily temperature rises above 0℃.',
                 cell_methods='',
...
cooling_degree_days = Tas(identifier='cddcold_{thresh}',
                          ...
                          description='{freq} number of degrees the daily mean temperature is above {thresh}°C.',
...
heating_degree_days = Tas(identifier='hddheat_{thresh}',
                          ...
                          description='{freq} number of degrees the daily mean temperature is below {thresh}°C.',
                          cell_methods='time: mean within days time: sum over days',
...
frost_days = Tasmin(identifier='frost_days',
                    ...
                    description='{freq} number of days with minimum daily temperature below 0°C.',
                    cell_methods='time: minimum within days time: sum over days',
...
ice_days = Tasmax(identifier='ice_days',
                  ...
                  description='{freq} number of days with maximum daily temperature below 0°C.',
                  cell_methods='time: maximum within days time: sum over days',
...
growing_season_length = Tas(identifier='gsl_{thresh}',
                            ...
                            description='{freq} number of days between the first occurrence of at least '
                                        ...
                                        'mean daily temperature below {thresh}℃ after July 1st.',
                            cell_methods='',
                            compute=_ind.growing_season_length,
                            )
# in the northern hemisphere and January 1st in the southern hemisphere. not implemented.

tropical_nights = Tasmin(identifier='tr_{thresh}',
                         units='days',
                         standard_name='number_of_days_with_air_temperature_above_threshold',
                         long_name='Number of Tropical Nights (Tmin > {thresh}C)',
                         description='{freq} number of days with minimum daily temperature above {thresh}℃.',
                         cell_methods='time: minimum within days time: sum over days',
...
tx90p = Tasmax(identifier='tx90p',
               ...
               description='{freq} number of days with maximum daily temperature above the 90th percentile.'
                           'The 90th percentile is to be computed for a 5 day moving window centered on each calendar '
                           'day for a reference period.',
               cell_methods='time: maximum within days time: sum over days',
               compute=_ind.tx90p,
               )
...
tx10p = Tasmax(identifier='tx10p',
               ...
               description='{freq} number of days with maximum daily temperature below the 10th percentile.'
                           'The 10th percentile is to be computed for a 5 day moving window centered on each calendar '
                           'day for a reference period.',
               cell_methods='time: maximum within days time: sum over days',
               compute=_ind.tx10p
               )
